...
# 묶음을 합쳐서 하나로 만드는데 비교를 왜하냐? 뭔소린지 모르겠는데 일단 더한다는 뜻으로 받아들이면 될 듯
# 풀이: 오름차순으로 정렬 후 (10 + 20) + ((10 + 20) + 30) + ((10 + 20 + 30) + 40) + ...  이런 식으로 더해 가면 됨
# 즉, 
# index = 0, 1 => n-1회
# index = 2 => n-2회
# index = 3 => n-3회
...
card_piles = []
for i in range(n):
    card_pile = int(sys.stdin.readline())
    card_piles.append(card_pile)

sorted_piles = sorted(card_piles)
print('sorted_piles:', sorted_piles)
result = 0
for idx, val in enumerate(sorted_piles):
    result += val * (n - idx)
    print('result:', result)
result -= sorted_piles[0]
print(result)
