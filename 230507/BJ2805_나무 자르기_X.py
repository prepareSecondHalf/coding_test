...
result = 0 # result를 갱신해서 최댓값을 찾아야 함
while start <= end:
    middle = (start + end) // 2
    ...
    for tree in trees:
        if tree - middle > 0:
            total_height += tree - middle
    
    if total_height >= m: # 가져가기 가능
        result = middle # 현재 높이 저장
        start = middle + 1 # middle의 최댓값을 구하기 위해 한 번 더 이진 탐색
    else: # 가져가기 불가능
        end = middle - 1
print(result)