...
# 풀이(벨만-포드 알고리즘)
# 웜홀 그래프 작성
def create_graph(n, roads, wormholes):
    graph = [[] for _ in range(n + 1)]
    # 도로
    for road in roads:
        s, e, t = road
        graph[s].append((e, t))
        graph[e].append((s, t))  # 양방향이므로
    # 웜홀
    for wormhole in wormholes:
        s, e, t = wormhole
        graph[s].append((e, -t))  # 단방향이므로
...
for _ in range(tc):
    ...
    graph = create_graph(n, roads, wormholes)
...
#######################################################################################
# 풀이: 음으로 갈 수 있으므로 플로이드 워셜...인 줄 알았는데 역시나 시간 초과
# 벨만 포드 알고리즘이라고 함
# tc = int(sys.stdin.readline())
